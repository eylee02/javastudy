...
public class Ex01_selectOne {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("조회할 UESR_NO >>>");
    int user_no = sc.nextInt();
    sc.close();
    
    // Connection 객체 선언 (DB 접속용)
    Connection con = null;
    ...
    try {
      ...
      // 쿼리문에 변수 넣기
      ps.setInt(1, user_no); // 1번째 물음표에 user_no값 넣기
      
      // 쿼리문 실행 ResultSet 반환(검색결과 처리값)
      // executeQuery : select 문을 실행할때 사용되는함수
      rs = ps.executeQuery();
